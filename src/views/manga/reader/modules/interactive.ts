...
export class InteractionManager {
  ...
  constructor (param: {
    ...
    dontBindEventsOnBody?: boolean
  }) {
    const {
      renderer, camera, domElement,
      usePointerEvents, dontBindEventsOnBody
    } = param
  ...
  onMouseDown = (mouseEvent: MouseEvent | PointerEvent) => {
    ...
    const event = new InteractiveEvent(
      this.supportsPointerEvents ? 'pointerdown' : 'mousedown',
      mouseEvent
    )
